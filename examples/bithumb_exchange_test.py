...
from src.exchanges.bithumb.bithumb_rest import BithumbRest, _to_market_code
...
def _find_deposit_address_pair(
    ...
    *,
    ...
    preferred_net_type: str | None,
) -> dict[str, str | None] | None:
    if not isinstance(addresses, list):
        return None

    preferred_currency_upper = preferred_currency.upper()
    preferred_net_type_upper = preferred_net_type.upper() if preferred_net_type else None

    for item in addresses:
        if not isinstance(item, dict):
            continue
        currency = str(item.get("currency") or "").upper()
        net_type_value = item.get("net_type")
        net_type = str(net_type_value).upper() if net_type_value else None
        if not currency:
            continue

        if currency != preferred_currency_upper:
            continue
        if preferred_net_type_upper is not None and net_type != preferred_net_type_upper:
            continue
...
def _slug(value: str) -> str:
    replacements = {
        ...
        "코인 출금 목록 조회": "coin_withdraws",
        "원화 출금 목록 조회": "krw_withdraws",
        "입금 주소 목록 조회": "deposit_addresses",
        "입금 주소 조회": "deposit_address",
        "코인 입금 목록 조회": "coin_deposits",
        "원화 입금 목록 조회": "krw_deposits",
...
def _print_summary(summary: dict[str, Any]) -> None:
    print("\n" + "=" * 80)
    print("[요약] 빗썸 Exchange API 테스트 결과")
    ...
    *,
    ...
    steps: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "method": "GET",
            "endpoint": "/v1/orders",
            "params": {"market": market, "state": "wait", "limit": 3},
            "call": lambda: client.get_open_orders(ticker=ticker, limit=3),
        },
        {
            "name": "주문 리스트 조회",
            "method": "GET",
            ...
            "call": lambda: client.list_orders(ticker=ticker, limit=3),
        },
        {
            "name": "지갑 상태 조회",
            "method": "GET",
        ...
        },
        {
            ...
            "call": lambda: client.get_withdraw_chance(
        ...
        },
        {
            ...
            "params": {"currency": base_currency, "net_type": net_type},
            "call": lambda: client.list_withdraw_addresses(
                currency=base_currency,
                net_type=net_type,
            ),
        },
        {
            ...
            "params": {"currency": base_currency, "limit": 3},
            "call": lambda: client.list_coin_withdraws(currency=base_currency, limit=3),
        },
        {
            "name": "원화 출금 목록 조회",
            "method": "GET",
            ...
            "params": {"limit": 3},
            "call": lambda: client.list_krw_withdraws(limit=3),
        },
        {
            "name": "입금 주소 목록 조회",
            "method": "GET",
        ...
        },
        {
            ...
            "params": {"currency": base_currency, "limit": 3},
            "call": lambda: client.list_coin_deposits(currency=base_currency, limit=3),
        },
        {
            "name": "원화 입금 목록 조회",
            "method": "GET",
            ...
            "params": {"limit": 3},
            "call": lambda: client.list_krw_deposits(limit=3),
        },
    ]

    artifacts: list[dict[str, Any]] = []
    deposit_address_pair: dict[str, str | None] | None = {
        "currency": deposit_currency.upper(),
        "net_type": deposit_net_type.upper() if deposit_net_type else None,
    }
    ...
    for step in steps:
        ...
        if step["name"] == "입금 주소 조회":
            if deposit_address_pair is None:
                executed = False
                requested_pair = (
                    f"{deposit_currency.upper()}/"
                    f"{deposit_net_type.upper() if deposit_net_type else 'None'}"
                )
                skip_reason = f"입금 주소 목록에 {requested_pair} 주소가 없어 단건 주소 조회를 건너뜁니다."
                params = {"currency": deposit_currency.upper(), "net_type": deposit_net_type}
                call = lambda: None
            else:
                selected_currency = str(deposit_address_pair["currency"])
                selected_net_type = deposit_address_pair["net_type"]
                params = {"currency": selected_currency, "net_type": selected_net_type}
        ...
        if step["name"] == "입금 주소 목록 조회":
            if artifact.get("success") is True:
                ...
                if deposit_address_pair is None:
                    ...
                    print(f"[참고] 입금 주소 목록에 {requested_pair} 주소가 없습니다.")
    ...
    *,
    ...
    if krw_before < trade_krw:
        artifact, _ = _run_step(
            ...
            method="POST",
            endpoint="/v2/orders",
            params={"market": market, "side": "bid", "order_type": "price", "price": str(trade_krw)},
            call=lambda: None,
            result_dir=result_dir,
            executed=False,
    ...
    if bought_delta <= 0:
        artifact, _ = _run_step(
            ...
            method="POST",
            endpoint="/v2/orders",
            params={"market": market, "side": "ask", "order_type": "market", "volume": str(bought_delta)},
            call=lambda: None,
            result_dir=result_dir,
            executed=False,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bithumb Exchange API live tester.")
    ...
    parser.add_argument(
        "--result-dir",
        default="tests/results/bithumb/exchange",
        help="directory where timestamped result artifacts are stored",
    )
    args = parser.parse_args()

    source: CredentialSource = args.source
    client = BithumbRest.from_config(source=source, file_path=args.info)
    ...
    deposit_net_type = (
        args.deposit_net_type.upper()
        if args.deposit_net_type is not None
        else (args.net_type.upper() if args.net_type else None)
    )
    result_dir = _new_result_dir(PROJECT_ROOT / args.result_dir)

    print("[시작] 빗썸 Exchange API 테스트")
    ...
    print("[안전장치] 출금/전송/전량매도/일괄취소/다건주문 API는 실행하지 않습니다.")
