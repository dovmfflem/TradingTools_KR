...
from src.core.event_constants import EXCHANGE_BINANCE_SPOT, SOURCE_ORDERBOOK
...
class BinanceSpotDataBank:
    ...
    def _build_targets(tickers: list[str], quote: str) -> dict[str, _TickerTarget]:
        ...
        for raw in tickers:
            ...
            key = base_coin.lower()
            targets[key] = _TickerTarget(key=key, symbol=symbol, stream=f"{symbol.lower()}@depth20@100ms")
