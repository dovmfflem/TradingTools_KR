...
from src.core.event_constants import EXCHANGE_BYBIT_SPOT, SOURCE_ORDERBOOK
...
class BybitSpotDataBank:
    ...
    def _ws_loop(self) -> None:
        if websocket is None:
            return
        args = [f"orderbook.50.{t.symbol}" for t in self._targets.values()]
    ...
    def _handle_ws_message(self, msg: dict[str, Any]) -> None:
        ...
        asks = self._normalize_levels(data.get("a") or data.get("asks"), self.count)
        bids = self._normalize_levels(data.get("b") or data.get("bids"), self.count)
        if asks or bids:
    ...
    def _rest_fallback_loop(self) -> None:
        while not self._stop_event.wait(self.rest_check_interval_seconds):
            ...
            for key in stale_keys:
                ...
                asks = self._normalize_levels(result.get("a") or result.get("asks"), self.count)
                bids = self._normalize_levels(result.get("b") or result.get("bids"), self.count)
                if asks or bids:
                    self._update_book(key, asks, bids)

    def _fetch_rest_orderbook(self, symbol: str) -> dict[str, Any]:
        limit = self._rest_limit(self.count)
        query = urllib_parse.urlencode({"category": "spot", "symbol": symbol, "limit": str(limit)})
...
if __name__ == "__main__":
    ...
    bank = BybitSpotDataBank(["btc-usdt", "eth-usdt"], count=5, event_queue=q)
    try:
        started = time.time()
