...
from src.core.event_constants import EXCHANGE_BYBIT_FUTURES, SOURCE_ORDERBOOK
...
class BybitFuturesDataBank:
    ...
    def _fetch_rest_orderbook(self, symbol: str) -> dict[str, Any]:
        ...
        query = urllib_parse.urlencode({"category": "linear", "symbol": symbol, "limit": str(limit)})
    ...
    def _update_book(self, key: str, asks: list[dict[str, float]], bids: list[dict[str, float]]) -> None:
        ...
        event = {
            ...
            "exchange": EXCHANGE_BYBIT_FUTURES,
            "ticker": key,
            "asks": asks,
            "bids": bids,
...
if __name__ == "__main__":
    ...
    bank = BybitFuturesDataBank(["btc-usdt", "eth-usdt"], count=5, event_queue=q)
    try:
        started = time.time()
