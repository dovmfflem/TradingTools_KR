...
class UpbitRestError(ExchangeRequestError):
    """Raised when an Upbit REST API request fails."""

    def __init__(self, exchange: str, code: str | None = None, **metadata):
        super().__init__("upbit", code or "LEGACY_RESPONSE_INVALID", **metadata)
...
class UpbitRest(ExchangeResponseMixin):
    def __init__(
        self,
        api_key: str,
        secret_key: str,
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise UpbitRestError("upbit", "TRANSPORT_FAILED", outcome_unknown=method != "GET") from None

        self._remember_response(response)
        status = response.status_code
        retry_after, remaining = response_metadata(response)
        if 300 <= status < 400:
            raise UpbitRestError("upbit", "REDIRECT_REJECTED", status_code=status,
                                 rate_limit=remaining, outcome_unknown=method != "GET")

        try:
            payload = response.json()
        except ValueError:
            raise UpbitRestError("upbit", "INVALID_RESPONSE", status_code=status,
                                 retry_after_seconds=retry_after, rate_limit=remaining,
                                 outcome_unknown=method != "GET") from None

        if not response.ok:
            raise UpbitRestError("upbit", safe_code(payload) if status != 429 else "RATE_LIMITED",
    ...
    def _public_request(
        ...
        *,
        ...
        try:
            ...
            raise UpbitRestError("upbit", "TRANSPORT_FAILED") from None

        self._remember_response(response)
        status = response.status_code
        retry_after, remaining = response_metadata(response)
        if 300 <= status < 400:
            raise UpbitRestError("upbit", "REDIRECT_REJECTED", status_code=status, rate_limit=remaining)
