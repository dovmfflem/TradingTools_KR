...
from src.core.event_constants import EXCHANGE_BITGET_FUTURES, SOURCE_ORDERBOOK
...
class BitgetFuturesDataBank:
    WS_URL = "wss://ws.bitget.com/v2/ws/public"
    REST_ORDERBOOK_URL = "https://api.bitget.com/api/v2/mix/market/orderbook"
    ...
    def _ws_loop(self) -> None:
        if websocket is None:
            return
        args = [{"instType": "USDT-FUTURES", "channel": "books5", "instId": t.symbol} for t in self._targets.values()]
    ...
    def _fetch_rest_orderbook(self, symbol: str) -> dict[str, Any]:
        ...
        query = urllib_parse.urlencode(
            {"symbol": symbol, "productType": "USDT-FUTURES", "precision": "scale0", "limit": str(limit)}
        )
    ...
    def _update_book(self, key: str, asks: list[dict[str, float]], bids: list[dict[str, float]]) -> None:
        ...
        event = {
            ...
            "exchange": EXCHANGE_BITGET_FUTURES,
            "ticker": key,
            "asks": asks,
            "bids": bids,
...
if __name__ == "__main__":
    ...
    bank = BitgetFuturesDataBank(["btc-usdt", "eth-usdt"], count=5, event_queue=q)
    try:
        started = time.time()
